...
#define DROPDOWNVIEW_HEADER_HEIGHT 45.
#define RADIUS 0
...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        //---
        [_kTableView setSeparatorColor:[UIColor grayColor]];
        [_kTableView setScrollEnabled:NO];
        //---
...
- (void)fadeIn
{
    ...
    self.frame = CGRectMake(tempRect.origin.x, tempRect.origin.y, tempRect.size.width, 0);
    
    self.transform = CGAffineTransformMakeScale(1.0, 0.5);
    self.alpha = 0;
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"chooseRandomMood" object:nil];    
}
